...
export default function LoginPage() {
  ...
  async function signInWithApple() {
    ...
    const safeReturnTo = returnTo?.startsWith('/') ? returnTo : '/dashboard';
    try {
      const { data, error } = await supabaseOAuth.auth.signInWithOAuth({ provider: 'apple', options: { redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(safeReturnTo)}`, skipBrowserRedirect: true } });
  ...
  return (
    <main className='auth-shell'>
      ...
      <form onSubmit={submit} className='auth-card auth-form' name='login'>
        ...
        <button className='auth-google-button auth-apple-button' type='button' onClick={signInWithApple} disabled={submitting}><AppleMark />Continue with Apple</button>
